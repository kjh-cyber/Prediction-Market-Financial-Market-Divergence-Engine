...
def build_pdf(S, rel_df, base_rate, b_model, b_clim, skill):
    ...
    _bullets(fig, [
        (0, "핵심 발견"),
        (1, "예측시장(Polymarket) 확률은 실제 결과와 잘 맞음 — 그림의 점들이 대각선에 밀착"),
        (1, "완만한 편향: 낮은 확률은 저평가·높은 확률은 고평가(이른바 favorite-longshot 편향)"),
        (0, "숫자로"),
        (1, f"예측 오차(Brier) {b_model:.3f} < 기준선(무조건 평균빈도로 찍기) {b_clim:.3f}"),
        (1, f"개선폭(skill) {skill:+.3f} → 시장이 단순 기준선보다 확실히 더 정확"),
        (1, f"기저율(YES 비율) {base_rate:.2f} — '안 일어남'으로 끝난 마켓이 많아 낮음"),
        (0, "한계 (해석 주의)"),
        *([
            (2, "같은 마켓의 시점들은 서로 비슷함 → 실질 표본 수는 관측 수보다 적음"),
            (2, "표본이 스포츠·낮은확률 마켓에 치우침 → 결과·확률이 낮은 쪽에 몰림"),
            (2, "해결 직전(확률이 이미 0/1에 가까움) 구간이 오차를 실제보다 좋게 보이게 함"),
          ] if backfilled else [
            (2, f"결과 확정 마켓 {S['resolved_markets']}개뿐 — 통계적으로 얇고 치우침"),
            (2, "결과를 최종 극단확률로 추정 — 공식 정산이 아님"),
          ]),
        (0, "다음에 하면 좋을 것"),
        (2, "중간 확률(40~70%) 마켓을 더 모아 높은확률 구간 신뢰도 보강"),
        (2, "분야별(정치/스포츠/크립토) 캘리브레이션 비교 — 편향이 어디서 오는지 규명"),
    ], y0=0.90, dy=0.029)
