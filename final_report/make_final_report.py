...
def main():
    ...
    _bullets(fig, [
        ...
        (1, "'예(YES)' 지분 가격이 곧 일어날 확률 (가격 0.60 = 확률 60%)"),
    ...
    _bullets(fig, [
        (0, "결과"),
        (1, "+6/24/72시간 모두 적중률 50.6~50.8% — 기준선 50%와 사실상 차이 없음"),
        (1, "BUY NO만 50% 넘고 BUY YES는 50% 미만 → 대칭임"),
        (2, "이건 신호가 아니라 '확률이 시간이 갈수록 NO로 기우는' 기저효과일 뿐"),
    ...
    _bullets(fig, [
        ...
        (1, "낮은 확률(15~25%)은 실제론 더 자주 일어남(저평가)"),
        (1, "높은 확률(75~85%)은 실제론 덜 일어남(고평가) — 극단서 살짝 과신"),
    ], y0=0.86)
    ...
    _bullets(fig, [
        (0, "읽는 법"),
        (1, "결과(YES/NO)별로 묶어 '남은 일수'에 맞춰 평균낸 확률 (오른쪽=해결)"),
        (0, "관찰"),
        (1, "YES 판명(초록)은 다가올수록 확률 상승 · NO 판명(빨강)은 낮게 유지"),
        (1, "→ 두 선이 점점 갈라짐 (시장이 시간이 갈수록 정답 방향으로 이동)"),
    ], y0=0.86)
    ...
    _bullets(fig, [
        ...
        (0, "한계 (과잉해석 금지)"),
        (2, "두 연구는 표본이 다름(예측력 19개 vs 캘리브레이션 1,200개)"),
        (2, "→ 정합적인 큰 그림이지, 동일 표본으로 맞물려 증명한 건 아님"),
